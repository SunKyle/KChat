...
SELECT 'default-system-prompt', 'default-system-prompt', 
       @default_system_prompt_v6,
       '默认系统提示词模板（v6：语言并入用户档案、移除档案双源与图谱推理、精简行为准则）', 'system', '{"user_profile": "无", "memory_cognee_graph": "无", "memory_l3_preference": "无", "memory_precise": "无", "context_policy": "", "search_context": "无", "custom_rules": ""}', 6, TRUE, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM prompt_templates WHERE name = 'default-system-prompt');

-- 升级旧版本模板（幂等：仅升级低于 v6 的模板，不覆盖用户自定义的新版本）
UPDATE prompt_templates
SET content = @default_system_prompt_v6, version = 6, active = TRUE, updated_at = NOW()
WHERE name = 'default-system-prompt' AND version < 6;
