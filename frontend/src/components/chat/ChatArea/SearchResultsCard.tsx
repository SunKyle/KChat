...
export function SearchResultsCard({ results }: SearchResultsCardProps) {
  ...
  return (
    <div className='mx-auto max-w-[85%] mb-3'>
      <div className='rounded-xl border border-[var(--accent-sky)]/20 bg-[var(--accent-sky)]/5 overflow-hidden'>
        <button
          onClick={() => setExpanded(!expanded)}
          className='w-full flex items-center gap-2 px-4 py-2.5 hover:bg-[var(--accent-sky)]/10 transition-colors'
        >
          <Globe className='w-3.5 h-3.5 text-[var(--accent-sky)]' />
          <span className='text-xs font-medium text-[var(--accent-sky)]'>
            搜索到 {snippets.length} 个相关网页
          </span>
          <ChevronDown
            className={`w-3.5 h-3.5 text-[var(--accent-sky)]/60 ml-auto transition-transform duration-200 ${
              expanded ? 'rotate-180' : ''
            }`}
          />
        </button>
          ...
          <div className='px-4 pb-3 space-y-2'>
            {snippets.map((item, i) => (
              <a
                ...
                className='block p-2.5 rounded-lg bg-[var(--bg-card)]/70 hover:bg-[var(--bg-card)] border border-[var(--border-secondary)] hover:border-[var(--accent-sky)]/30 transition-all group'
              >
                <div className='flex items-start gap-2'>
                  <div className='flex-1 min-w-0'>
                    <div className='flex items-center gap-1.5'>
                      ...
                      <ExternalLink className='w-3 h-3 text-[var(--text-muted)] group-hover:text-[var(--accent-sky)] flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity' />
                    </div>
