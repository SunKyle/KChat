...
function GraphInner() {
  ...
  const fetchGraph = useCallback(async () => {
    ...
    try {
      ...
      const rfEdges: Edge[] = data.edges.map((e: GraphEdge) => ({
        ...
        type: 'smoothstep',
        animated: false,
        style: { strokeWidth: 1.5, stroke: 'var(--border-secondary)' },
        labelStyle: { fill: 'var(--text-muted)', fontSize: 10 },
        labelBgStyle: { fill: 'var(--bg-input)' },
      }))
  ...
  const filteredEdges = useMemo(() => {
    ...
    return edges.map((e) => ({
      ...e,
      hidden: !filteredNodeIds.has(e.source) || !filteredNodeIds.has(e.target),
    }))
    ...
    <div className="relative w-full h-[600px] rounded-xl border border-[var(--border-secondary)] overflow-hidden bg-[var(--bg-card)]">
      ...
      <Panel position="bottom-left" className="!m-2">
        <div className="bg-[var(--bg-card)]/95 backdrop-blur rounded-xl border border-[var(--border-secondary)] px-3 py-2 shadow-lg">
          <div className="text-xs font-semibold text-[var(--text-secondary)] mb-1.5">类型图例</div>
      ...
      {error && (
        <div className="absolute inset-0 flex items-center justify-center bg-[var(--bg-card)]/80 backdrop-blur-sm z-10">
          <div className="text-center p-6 bg-[var(--bg-card)] rounded-2xl border border-red-500/30 shadow-xl max-w-md">
            ...
            <h3 className="text-base font-semibold text-[var(--text-primary)] mb-1">图谱加载失败</h3>
      ...
      >
        ...
        <Controls
          className="!bg-[var(--bg-card)] !border-[var(--border-secondary)] [&>button]:!bg-[var(--bg-card)] [&>button]:!border-[var(--border-secondary)] [&>button]:!text-[var(--text-secondary)] hover:[&>button]:!bg-[var(--bg-hover)]"
        />
      </ReactFlow>
            ...
            <div>
              ...
              <div className="mt-1.5 space-y-1">
                {edges.filter((e) => e.source === selectedNode.id || e.target === selectedNode.id).map((e) => {
                  ...
                  return (
                    <div key={e.id} className="flex items-center gap-1 text-xs text-[var(--text-secondary)]">
