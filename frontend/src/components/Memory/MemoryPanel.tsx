    ...
    <div className="space-y-4">
      ...
      {selectedMemories.length > 0 && (
        <div className="flex items-center gap-3 px-3 py-2 bg-slate-800/20 rounded-lg">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-slate-400 text-xs">已选 {selectedMemories.length}</span>
          </label>
          <button
        ...
        {loading ? (
          ...
          filteredMemories.map(memory => {
            ...
            return (
              <div
                key={memory.id}
                className={`group relative flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer transition-all duration-200 ${
                  isSelected
                    ? 'bg-blue-600/25 border-l-4 border-blue-500 shadow-md shadow-blue-500/20'
              ...
              >
                ...
                {/* 类型图标 */}
                <TypeIcon className={`w-4 h-4 transition-all ${isSelected ? 'text-blue-400 scale-110' : colorClass}`} />
                ...
                <div className="flex items-center gap-2">
                  {memory.isRule && (
                    <span className={`px-2 py-0.5 rounded text-[10px] font-semibold ${
                      isSelected ? 'bg-red-500/30 text-red-300' : 'bg-red-500/20 text-red-400'
                    }`}>规则</span>
                  )}
                ...
                <div className={`flex items-center gap-1 transition-opacity ${
                  ...
                  <button
                    ...
                    className="p-1.5 hover:bg-red-500/20 rounded-lg text-slate-300 hover:text-red-400 transition-colors"
                    title="删除"
                  >
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => { setShowForm(false); setEditingMemory(null); }}>
          <div className="w-full max-w-md bg-[#1E293B] rounded-xl border border-white/10 overflow-hidden" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b border-white/5">
              <h3 className="text-sm font-medium text-white">{editingMemory ? '编辑记忆' : '添加记忆'}</h3>
              <button onClick={() => { setShowForm(false); setEditingMemory(null); }} className="p-1 text-slate-400 hover:text-white">
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-3.5 h-3.5 rounded border-white/10"
                />
                <label htmlFor="isRule" className="text-xs text-slate-400 cursor-pointer">标记为规则</label>
              </div>
            </div>
