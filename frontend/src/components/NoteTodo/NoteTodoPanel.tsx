            ...
            {selectedNote ? (
              ...
              <div className='flex-1 flex flex-col overflow-hidden'>
                ...
                <div className='flex-1 overflow-y-auto p-4 space-y-4'>
                  <div>
                    <label className='block text-[11px] font-semibold text-[var(--text-muted)] uppercase tracking-wider mb-1.5'>标题</label>
                    <input type='text' value={formState.title} onChange={(e) => setFormState((p) => ({ ...p, title: e.target.value }))} placeholder={mode === 'note' ? '输入笔记标题...' : '输入待办标题...'} className='w-full px-3 py-2.5 bg-[var(--bg-input)] border border-[var(--border-primary)] rounded-xl text-[14px] text-[var(--text-primary)] placeholder:text-[var(--text-placeholder)] focus:outline-none focus:border-[var(--brand-primary)]/50 transition-colors' autoFocus />
                  </div>
                  {mode === 'note' && (
                    <div>
                      <label className='block text-[11px] font-semibold text-[var(--text-muted)] uppercase tracking-wider mb-1.5'>内容</label>
                      <textarea value={formState.content} onChange={(e) => setFormState((p) => ({ ...p, content: e.target.value }))} placeholder='开始记录...' rows={8} className='w-full px-3 py-2.5 bg-[var(--bg-input)] border border-[var(--border-primary)] rounded-xl text-[14px] text-[var(--text-primary)] placeholder:text-[var(--text-placeholder)] focus:outline-none focus:border-[var(--brand-primary)]/50 transition-colors resize-none leading-relaxed' />
                    </div>
                  )}
                  {mode === 'todo' && (
                    <div>
                      <label className='block text-[11px] font-semibold text-[var(--text-muted)] uppercase tracking-wider mb-1.5'>描述</label>
                      <textarea value={formState.description} onChange={(e) => setFormState((p) => ({ ...p, description: e.target.value }))} placeholder='添加描述（可选）' rows={4} className='w-full px-3 py-2.5 bg-[var(--bg-input)] border border-[var(--border-primary)] rounded-xl text-[14px] text-[var(--text-primary)] placeholder:text-[var(--text-placeholder)] focus:outline-none focus:border-[var(--brand-primary)]/50 transition-colors resize-none leading-relaxed' />
                    </div>
                  )}
                  <div className='grid grid-cols-2 gap-3'>
                    <div>
                ...
                <div className='flex-shrink-0 flex items-center justify-end gap-2 px-4 py-3 border-t border-[var(--border-divider)]'>
                  <button onClick={handleCancelForm} className='px-4 py-2 bg-[var(--bg-hover)] text-[var(--text-secondary)] rounded-xl text-[13px] font-medium hover:bg-[var(--bg-input)] transition-colors'>取消</button>
                  <button onClick={mode === 'note' ? handleCreateNote : handleCreateTodo} className='px-5 py-2 rounded-xl text-[13px] font-medium text-white bg-[var(--brand-primary)] hover:brightness-110 transition-all'>创建</button>
                </div>
              </div>
            ) : (
